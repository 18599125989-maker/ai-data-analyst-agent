...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_campaign_attribution.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_campaign_attribution_validation.json
└── fact_campaign_attribution_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_campaign_attribution_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_campaign_attribution_validation.md"

TABLE_NAME = 'fact_campaign_attribution'
PRIMARY_KEY = ['user_id', 'campaign_id', 'touch_type', 'touch_at']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'user_id', 'ref_table': 'dim_user', 'ref_col': 'user_id'}, {'column': 'campaign_id', 'ref_table': 'fact_campaign', 'ref_col': 'campaign_id'}]
ENUM_COLUMNS = ['touch_type']
TIME_COLUMNS = ['touch_at']
NUMERIC_COLUMNS = ['is_converted']
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    touch_at AS time_key,\n    COUNT(*) AS row_count\nFROM fact_campaign_attribution\nGROUP BY touch_at\nORDER BY touch_at;', 'enum_distribution': 'SELECT\n    touch_type,\n    COUNT(DISTINCT user_id) AS entity_count\nFROM fact_campaign_attribution\nGROUP BY touch_type\nORDER BY entity_count DESC, touch_type;', 'metric_trend': 'SELECT\n    touch_at AS time_key,\n    ROUND(SUM(is_converted), 2) AS total_is_converted\nFROM fact_campaign_attribution\nGROUP BY touch_at\nORDER BY touch_at;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_campaign_attribution base\nLEFT JOIN dim_user ref\n    ON base.user_id = ref.user_id;'}
