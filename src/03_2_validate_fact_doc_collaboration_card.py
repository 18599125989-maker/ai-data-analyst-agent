...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_doc_collaboration.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_doc_collaboration_validation.json
└── fact_doc_collaboration_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_doc_collaboration_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_doc_collaboration_validation.md"

TABLE_NAME = 'fact_doc_collaboration'
PRIMARY_KEY = ['doc_id', 'user_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'doc_id', 'ref_table': 'fact_document', 'ref_col': 'doc_id'}, {'column': 'user_id', 'ref_table': 'dim_user', 'ref_col': 'user_id'}]
ENUM_COLUMNS = []
TIME_COLUMNS = ['dt']
NUMERIC_COLUMNS = ['edit_duration_sec', 'comment_count']
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    dt AS time_key,\n    COUNT(*) AS row_count\nFROM fact_doc_collaboration\nGROUP BY dt\nORDER BY dt;', 'metric_trend': 'SELECT\n    dt AS time_key,\n    ROUND(SUM(edit_duration_sec), 2) AS total_edit_duration_sec\nFROM fact_doc_collaboration\nGROUP BY dt\nORDER BY dt;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_doc_collaboration base\nLEFT JOIN fact_document ref\n    ON base.doc_id = ref.doc_id;'}
