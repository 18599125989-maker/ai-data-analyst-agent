...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_tenant_metrics_snapshot.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_tenant_metrics_snapshot_validation.json
└── fact_tenant_metrics_snapshot_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_tenant_metrics_snapshot_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_tenant_metrics_snapshot_validation.md"

TABLE_NAME = 'fact_tenant_metrics_snapshot'
PRIMARY_KEY = ['tenant_id', 'snapshot_date']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'tenant_id', 'ref_table': 'dim_tenant', 'ref_col': 'tenant_id'}]
ENUM_COLUMNS = []
TIME_COLUMNS = ['snapshot_date']
NUMERIC_COLUMNS = ['total_users', 'total_docs', 'total_storage_mb', 'total_revenue', 'total_messages', 'active_users_today']
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    snapshot_date AS time_key,\n    COUNT(*) AS row_count\nFROM fact_tenant_metrics_snapshot\nGROUP BY snapshot_date\nORDER BY snapshot_date;', 'metric_trend': 'SELECT\n    snapshot_date AS time_key,\n    ROUND(SUM(total_users), 2) AS total_total_users\nFROM fact_tenant_metrics_snapshot\nGROUP BY snapshot_date\nORDER BY snapshot_date;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_tenant_metrics_snapshot base\nLEFT JOIN dim_tenant ref\n    ON base.tenant_id = ref.tenant_id;'}
