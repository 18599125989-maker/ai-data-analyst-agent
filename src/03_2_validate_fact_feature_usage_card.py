...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_feature_usage.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_feature_usage_validation.json
└── fact_feature_usage_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_feature_usage_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_feature_usage_validation.md"

TABLE_NAME = 'fact_feature_usage'
PRIMARY_KEY = ['user_id', 'feature_key', 'dt']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'user_id', 'ref_table': 'dim_user', 'ref_col': 'user_id'}, {'column': 'feature_key', 'ref_table': 'dim_feature', 'ref_col': 'feature_key'}]
ENUM_COLUMNS = ['feature_key']
TIME_COLUMNS = ['dt']
NUMERIC_COLUMNS = ['action_count', 'duration_sec']
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    dt AS time_key,\n    COUNT(*) AS row_count\nFROM fact_feature_usage\nGROUP BY dt\nORDER BY dt;', 'enum_distribution': 'SELECT\n    feature_key,\n    COUNT(DISTINCT user_id) AS entity_count\nFROM fact_feature_usage\nGROUP BY feature_key\nORDER BY entity_count DESC, feature_key;', 'metric_trend': 'SELECT\n    dt AS time_key,\n    ROUND(SUM(action_count), 2) AS total_action_count\nFROM fact_feature_usage\nGROUP BY dt\nORDER BY dt;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_feature_usage base\nLEFT JOIN dim_user ref\n    ON base.user_id = ref.user_id;'}
