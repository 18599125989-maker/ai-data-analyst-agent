...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_ticket.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_ticket_validation.json
└── fact_ticket_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_ticket_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_ticket_validation.md"

TABLE_NAME = 'fact_ticket'
PRIMARY_KEY = ['ticket_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'tenant_id', 'ref_table': 'dim_tenant', 'ref_col': 'tenant_id'}, {'column': 'category', 'ref_table': 'dim_ticket_category', 'ref_col': 'category_id'}]
ENUM_COLUMNS = ['category', 'priority', 'status']
TIME_COLUMNS = ['created_at', 'resolved_at']
NUMERIC_COLUMNS = []
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    created_at AS time_key,\n    COUNT(*) AS row_count\nFROM fact_ticket\nGROUP BY created_at\nORDER BY created_at;', 'enum_distribution': 'SELECT\n    category,\n    COUNT(DISTINCT ticket_id) AS entity_count\nFROM fact_ticket\nGROUP BY category\nORDER BY entity_count DESC, category;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_ticket base\nLEFT JOIN dim_tenant ref\n    ON base.tenant_id = ref.tenant_id;'}
