...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_experiment_metric.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_experiment_metric_validation.json
└── fact_experiment_metric_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_experiment_metric_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_experiment_metric_validation.md"

TABLE_NAME = 'fact_experiment_metric'
PRIMARY_KEY = ['exp_id', 'variant', 'metric_name', 'dt']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'exp_id', 'ref_table': 'fact_experiment', 'ref_col': 'exp_id'}]
ENUM_COLUMNS = ['variant', 'metric_name']
TIME_COLUMNS = ['dt']
NUMERIC_COLUMNS = ['sample_size', 'metric_value', 'ci_lower', 'ci_upper']
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    dt AS time_key,\n    COUNT(*) AS row_count\nFROM fact_experiment_metric\nGROUP BY dt\nORDER BY dt;', 'enum_distribution': 'SELECT\n    variant,\n    COUNT(DISTINCT exp_id) AS entity_count\nFROM fact_experiment_metric\nGROUP BY variant\nORDER BY entity_count DESC, variant;', 'metric_trend': 'SELECT\n    dt AS time_key,\n    ROUND(SUM(sample_size), 2) AS total_sample_size\nFROM fact_experiment_metric\nGROUP BY dt\nORDER BY dt;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_experiment_metric base\nLEFT JOIN fact_experiment ref\n    ON base.exp_id = ref.exp_id;'}
