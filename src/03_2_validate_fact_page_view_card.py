...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_page_view.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_page_view_validation.json
└── fact_page_view_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_page_view_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_page_view_validation.md"

TABLE_NAME = 'fact_page_view'
PRIMARY_KEY = ['pv_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'session_id', 'ref_table': 'fact_session', 'ref_col': 'session_id'}, {'column': 'user_id', 'ref_table': 'dim_user', 'ref_col': 'user_id'}]
ENUM_COLUMNS = ['page_path', 'referrer']
TIME_COLUMNS = []
NUMERIC_COLUMNS = ['view_duration_ms']
JSON_COLUMNS = []
SQL_PATTERNS = {'enum_distribution': 'SELECT\n    page_path,\n    COUNT(DISTINCT pv_id) AS entity_count\nFROM fact_page_view\nGROUP BY page_path\nORDER BY entity_count DESC, page_path;', 'metric_by_enum': 'SELECT\n    page_path,\n    ROUND(SUM(view_duration_ms), 2) AS total_view_duration_ms\nFROM fact_page_view\nGROUP BY page_path\nORDER BY total_view_duration_ms DESC, page_path;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_page_view base\nLEFT JOIN fact_session ref\n    ON base.session_id = ref.session_id;'}
