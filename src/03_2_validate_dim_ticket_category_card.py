...
"""
功能：
验证 knowledge_manual/table_cards_draft/dim_ticket_category.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── dim_ticket_category_validation.json
└── dim_ticket_category_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "dim_ticket_category_validation.json"
MD_OUTPUT = OUTPUT_DIR / "dim_ticket_category_validation.md"

TABLE_NAME = 'dim_ticket_category'
PRIMARY_KEY = ['category_id']
NATURAL_KEY = []
FOREIGN_KEYS = []
ENUM_COLUMNS = ['category_id', 'category_name', 'parent_category_id']
TIME_COLUMNS = []
NUMERIC_COLUMNS = ['sla_hours']
JSON_COLUMNS = []
SQL_PATTERNS = {'enum_distribution': 'SELECT\n    category_id,\n    COUNT(DISTINCT category_id) AS entity_count\nFROM dim_ticket_category\nGROUP BY category_id\nORDER BY entity_count DESC, category_id;', 'metric_by_enum': 'SELECT\n    category_id,\n    ROUND(SUM(sla_hours), 2) AS total_sla_hours\nFROM dim_ticket_category\nGROUP BY category_id\nORDER BY total_sla_hours DESC, category_id;'}
