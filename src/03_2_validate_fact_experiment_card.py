...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_experiment.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_experiment_validation.json
└── fact_experiment_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_experiment_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_experiment_validation.md"

TABLE_NAME = 'fact_experiment'
PRIMARY_KEY = ['exp_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'feature_key', 'ref_table': 'dim_feature', 'ref_col': 'feature_key'}]
ENUM_COLUMNS = ['feature_key', 'status']
TIME_COLUMNS = ['start_date', 'end_date']
NUMERIC_COLUMNS = []
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    start_date AS time_key,\n    COUNT(*) AS row_count\nFROM fact_experiment\nGROUP BY start_date\nORDER BY start_date;', 'enum_distribution': 'SELECT\n    feature_key,\n    COUNT(DISTINCT exp_id) AS entity_count\nFROM fact_experiment\nGROUP BY feature_key\nORDER BY entity_count DESC, feature_key;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_experiment base\nLEFT JOIN dim_feature ref\n    ON base.feature_key = ref.feature_key;'}
