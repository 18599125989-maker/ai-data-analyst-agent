...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_ticket_reply.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_ticket_reply_validation.json
└── fact_ticket_reply_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_ticket_reply_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_ticket_reply_validation.md"

TABLE_NAME = 'fact_ticket_reply'
PRIMARY_KEY = ['reply_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'ticket_id', 'ref_table': 'fact_ticket', 'ref_col': 'ticket_id'}]
ENUM_COLUMNS = ['author_type']
TIME_COLUMNS = ['replied_at']
NUMERIC_COLUMNS = ['content_length']
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    replied_at AS time_key,\n    COUNT(*) AS row_count\nFROM fact_ticket_reply\nGROUP BY replied_at\nORDER BY replied_at;', 'enum_distribution': 'SELECT\n    author_type,\n    COUNT(DISTINCT reply_id) AS entity_count\nFROM fact_ticket_reply\nGROUP BY author_type\nORDER BY entity_count DESC, author_type;', 'metric_trend': 'SELECT\n    replied_at AS time_key,\n    ROUND(SUM(content_length), 2) AS total_content_length\nFROM fact_ticket_reply\nGROUP BY replied_at\nORDER BY replied_at;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_ticket_reply base\nLEFT JOIN fact_ticket ref\n    ON base.ticket_id = ref.ticket_id;'}
