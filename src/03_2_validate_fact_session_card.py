...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_session.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_session_validation.json
└── fact_session_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_session_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_session_validation.md"

TABLE_NAME = 'fact_session'
PRIMARY_KEY = ['session_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'user_id', 'ref_table': 'dim_user', 'ref_col': 'user_id'}]
ENUM_COLUMNS = ['device', 'ip_country']
TIME_COLUMNS = ['start_time', 'end_time']
NUMERIC_COLUMNS = []
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    start_time AS time_key,\n    COUNT(*) AS row_count\nFROM fact_session\nGROUP BY start_time\nORDER BY start_time;', 'enum_distribution': 'SELECT\n    device,\n    COUNT(DISTINCT session_id) AS entity_count\nFROM fact_session\nGROUP BY device\nORDER BY entity_count DESC, device;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_session base\nLEFT JOIN dim_user ref\n    ON base.user_id = ref.user_id;'}
