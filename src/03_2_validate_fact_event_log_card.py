...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_event_log.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_event_log_validation.json
└── fact_event_log_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_event_log_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_event_log_validation.md"

TABLE_NAME = 'fact_event_log'
PRIMARY_KEY = ['event_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'user_id', 'ref_table': 'dim_user', 'ref_col': 'user_id'}]
ENUM_COLUMNS = ['event_name', 'page']
TIME_COLUMNS = ['event_time']
NUMERIC_COLUMNS = []
JSON_COLUMNS = ['properties_json']
SQL_PATTERNS = {'time_distribution': 'SELECT\n    event_time AS time_key,\n    COUNT(*) AS row_count\nFROM fact_event_log\nGROUP BY event_time\nORDER BY event_time;', 'enum_distribution': 'SELECT\n    event_name,\n    COUNT(DISTINCT event_id) AS entity_count\nFROM fact_event_log\nGROUP BY event_name\nORDER BY entity_count DESC, event_name;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_event_log base\nLEFT JOIN dim_user ref\n    ON base.user_id = ref.user_id;'}
