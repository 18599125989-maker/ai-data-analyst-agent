...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_invoice.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_invoice_validation.json
└── fact_invoice_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_invoice_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_invoice_validation.md"

TABLE_NAME = 'fact_invoice'
PRIMARY_KEY = ['invoice_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'tenant_id', 'ref_table': 'dim_tenant', 'ref_col': 'tenant_id'}, {'column': 'sub_id', 'ref_table': 'fact_subscription', 'ref_col': 'sub_id'}]
ENUM_COLUMNS = ['currency', 'status']
TIME_COLUMNS = ['issued_at', 'paid_at']
NUMERIC_COLUMNS = ['amount']
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    issued_at AS time_key,\n    COUNT(*) AS row_count\nFROM fact_invoice\nGROUP BY issued_at\nORDER BY issued_at;', 'enum_distribution': 'SELECT\n    currency,\n    COUNT(DISTINCT invoice_id) AS entity_count\nFROM fact_invoice\nGROUP BY currency\nORDER BY entity_count DESC, currency;', 'metric_trend': 'SELECT\n    issued_at AS time_key,\n    ROUND(SUM(amount), 2) AS total_amount\nFROM fact_invoice\nGROUP BY issued_at\nORDER BY issued_at;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_invoice base\nLEFT JOIN dim_tenant ref\n    ON base.tenant_id = ref.tenant_id;'}
