...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_message.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_message_validation.json
└── fact_message_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_message_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_message_validation.md"

TABLE_NAME = 'fact_message'
PRIMARY_KEY = ['msg_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'channel_id', 'ref_table': 'dim_channel', 'ref_col': 'channel_id'}]
ENUM_COLUMNS = ['msg_type']
TIME_COLUMNS = ['sent_at']
NUMERIC_COLUMNS = ['word_count']
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    sent_at AS time_key,\n    COUNT(*) AS row_count\nFROM fact_message\nGROUP BY sent_at\nORDER BY sent_at;', 'enum_distribution': 'SELECT\n    msg_type,\n    COUNT(DISTINCT msg_id) AS entity_count\nFROM fact_message\nGROUP BY msg_type\nORDER BY entity_count DESC, msg_type;', 'metric_trend': 'SELECT\n    sent_at AS time_key,\n    ROUND(SUM(word_count), 2) AS total_word_count\nFROM fact_message\nGROUP BY sent_at\nORDER BY sent_at;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_message base\nLEFT JOIN dim_channel ref\n    ON base.channel_id = ref.channel_id;'}
