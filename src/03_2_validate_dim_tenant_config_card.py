...
"""
功能：
验证 knowledge_manual/table_cards_draft/dim_tenant_config.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── dim_tenant_config_validation.json
└── dim_tenant_config_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "dim_tenant_config_validation.json"
MD_OUTPUT = OUTPUT_DIR / "dim_tenant_config_validation.md"

TABLE_NAME = 'dim_tenant_config'
PRIMARY_KEY = ['tenant_id', 'feature_key']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'tenant_id', 'ref_table': 'dim_tenant', 'ref_col': 'tenant_id'}, {'column': 'feature_key', 'ref_table': 'dim_feature', 'ref_col': 'feature_key'}]
ENUM_COLUMNS = ['feature_key', 'config_json']
TIME_COLUMNS = []
NUMERIC_COLUMNS = ['enabled']
JSON_COLUMNS = ['config_json']
SQL_PATTERNS = {'enum_distribution': 'SELECT\n    feature_key,\n    COUNT(DISTINCT tenant_id) AS entity_count\nFROM dim_tenant_config\nGROUP BY feature_key\nORDER BY entity_count DESC, feature_key;', 'metric_by_enum': 'SELECT\n    feature_key,\n    ROUND(SUM(enabled), 2) AS total_enabled\nFROM dim_tenant_config\nGROUP BY feature_key\nORDER BY total_enabled DESC, feature_key;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM dim_tenant_config base\nLEFT JOIN dim_tenant ref\n    ON base.tenant_id = ref.tenant_id;'}
