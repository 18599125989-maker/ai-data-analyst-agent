...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_payment.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_payment_validation.json
└── fact_payment_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_payment_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_payment_validation.md"

TABLE_NAME = 'fact_payment'
PRIMARY_KEY = ['payment_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'invoice_id', 'ref_table': 'fact_invoice', 'ref_col': 'invoice_id'}]
ENUM_COLUMNS = ['method', 'status']
TIME_COLUMNS = ['paid_at']
NUMERIC_COLUMNS = ['amount']
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    paid_at AS time_key,\n    COUNT(*) AS row_count\nFROM fact_payment\nGROUP BY paid_at\nORDER BY paid_at;', 'enum_distribution': 'SELECT\n    method,\n    COUNT(DISTINCT payment_id) AS entity_count\nFROM fact_payment\nGROUP BY method\nORDER BY entity_count DESC, method;', 'metric_trend': 'SELECT\n    paid_at AS time_key,\n    ROUND(SUM(amount), 2) AS total_amount\nFROM fact_payment\nGROUP BY paid_at\nORDER BY paid_at;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_payment base\nLEFT JOIN fact_invoice ref\n    ON base.invoice_id = ref.invoice_id;'}
