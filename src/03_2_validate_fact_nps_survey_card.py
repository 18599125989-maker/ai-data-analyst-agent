...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_nps_survey.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_nps_survey_validation.json
└── fact_nps_survey_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_nps_survey_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_nps_survey_validation.md"

TABLE_NAME = 'fact_nps_survey'
PRIMARY_KEY = ['survey_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'user_id', 'ref_table': 'dim_user', 'ref_col': 'user_id'}, {'column': 'tenant_id', 'ref_table': 'dim_tenant', 'ref_col': 'tenant_id'}, {'column': 'plan_tier', 'ref_table': 'dim_plan', 'ref_col': 'plan_tier'}]
ENUM_COLUMNS = ['score', 'plan_tier', 'feedback_text']
TIME_COLUMNS = ['survey_date']
NUMERIC_COLUMNS = ['role', 'is_completed']
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    survey_date AS time_key,\n    COUNT(*) AS row_count\nFROM fact_nps_survey\nGROUP BY survey_date\nORDER BY survey_date;', 'enum_distribution': 'SELECT\n    score,\n    COUNT(DISTINCT survey_id) AS entity_count\nFROM fact_nps_survey\nGROUP BY score\nORDER BY entity_count DESC, score;', 'metric_trend': 'SELECT\n    survey_date AS time_key,\n    ROUND(SUM(role), 2) AS total_role\nFROM fact_nps_survey\nGROUP BY survey_date\nORDER BY survey_date;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_nps_survey base\nLEFT JOIN dim_user ref\n    ON base.user_id = ref.user_id;'}
