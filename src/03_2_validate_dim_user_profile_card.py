...
"""
功能：
验证 knowledge_manual/table_cards_draft/dim_user_profile.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── dim_user_profile_validation.json
└── dim_user_profile_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "dim_user_profile_validation.json"
MD_OUTPUT = OUTPUT_DIR / "dim_user_profile_validation.md"

TABLE_NAME = 'dim_user_profile'
PRIMARY_KEY = ['user_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'user_id', 'ref_table': 'dim_user', 'ref_col': 'user_id'}]
ENUM_COLUMNS = ['language', 'timezone', 'device_os']
TIME_COLUMNS = []
NUMERIC_COLUMNS = []
JSON_COLUMNS = []
SQL_PATTERNS = {'enum_distribution': 'SELECT\n    language,\n    COUNT(DISTINCT user_id) AS entity_count\nFROM dim_user_profile\nGROUP BY language\nORDER BY entity_count DESC, language;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM dim_user_profile base\nLEFT JOIN dim_user ref\n    ON base.user_id = ref.user_id;'}
