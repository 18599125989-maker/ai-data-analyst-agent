...
"""
功能：
验证 knowledge_manual/table_cards_draft/dim_feature.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── dim_feature_validation.json
└── dim_feature_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "dim_feature_validation.json"
MD_OUTPUT = OUTPUT_DIR / "dim_feature_validation.md"

TABLE_NAME = 'dim_feature'
PRIMARY_KEY = ['feature_key']
NATURAL_KEY = []
FOREIGN_KEYS = []
ENUM_COLUMNS = ['feature_key', 'feature_name', 'module', 'category']
TIME_COLUMNS = ['launched_at']
NUMERIC_COLUMNS = []
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    launched_at AS time_key,\n    COUNT(*) AS row_count\nFROM dim_feature\nGROUP BY launched_at\nORDER BY launched_at;', 'enum_distribution': 'SELECT\n    feature_key,\n    COUNT(DISTINCT feature_key) AS entity_count\nFROM dim_feature\nGROUP BY feature_key\nORDER BY entity_count DESC, feature_key;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM dim_feature base\nLEFT JOIN dim_ticket_category ref\n    ON base.category = ref.category_id;'}
