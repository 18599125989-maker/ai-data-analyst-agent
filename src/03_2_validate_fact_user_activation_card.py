...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_user_activation.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_user_activation_validation.json
└── fact_user_activation_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_user_activation_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_user_activation_validation.md"

TABLE_NAME = 'fact_user_activation'
PRIMARY_KEY = ['user_id', 'milestone_type', 'reached_at']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'user_id', 'ref_table': 'dim_user', 'ref_col': 'user_id'}]
ENUM_COLUMNS = ['milestone_type']
TIME_COLUMNS = ['reached_at']
NUMERIC_COLUMNS = []
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    reached_at AS time_key,\n    COUNT(*) AS row_count\nFROM fact_user_activation\nGROUP BY reached_at\nORDER BY reached_at;', 'enum_distribution': 'SELECT\n    milestone_type,\n    COUNT(DISTINCT user_id) AS entity_count\nFROM fact_user_activation\nGROUP BY milestone_type\nORDER BY entity_count DESC, milestone_type;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_user_activation base\nLEFT JOIN dim_user ref\n    ON base.user_id = ref.user_id;'}
