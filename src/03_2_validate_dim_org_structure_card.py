...
"""
功能：
验证 knowledge_manual/table_cards_draft/dim_org_structure.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── dim_org_structure_validation.json
└── dim_org_structure_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "dim_org_structure_validation.json"
MD_OUTPUT = OUTPUT_DIR / "dim_org_structure_validation.md"

TABLE_NAME = 'dim_org_structure'
PRIMARY_KEY = ['node_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'tenant_id', 'ref_table': 'dim_tenant', 'ref_col': 'tenant_id'}]
ENUM_COLUMNS = ['node_type']
TIME_COLUMNS = []
NUMERIC_COLUMNS = ['level']
JSON_COLUMNS = []
SQL_PATTERNS = {'enum_distribution': 'SELECT\n    node_type,\n    COUNT(DISTINCT node_id) AS entity_count\nFROM dim_org_structure\nGROUP BY node_type\nORDER BY entity_count DESC, node_type;', 'metric_by_enum': 'SELECT\n    node_type,\n    ROUND(SUM(level), 2) AS total_level\nFROM dim_org_structure\nGROUP BY node_type\nORDER BY total_level DESC, node_type;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM dim_org_structure base\nLEFT JOIN dim_tenant ref\n    ON base.tenant_id = ref.tenant_id;'}
