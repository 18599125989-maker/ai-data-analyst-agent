...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_experiment_assignment.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_experiment_assignment_validation.json
└── fact_experiment_assignment_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_experiment_assignment_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_experiment_assignment_validation.md"

TABLE_NAME = 'fact_experiment_assignment'
PRIMARY_KEY = ['user_id', 'exp_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'user_id', 'ref_table': 'dim_user', 'ref_col': 'user_id'}, {'column': 'exp_id', 'ref_table': 'fact_experiment', 'ref_col': 'exp_id'}]
ENUM_COLUMNS = ['variant']
TIME_COLUMNS = ['assigned_at']
NUMERIC_COLUMNS = []
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    assigned_at AS time_key,\n    COUNT(*) AS row_count\nFROM fact_experiment_assignment\nGROUP BY assigned_at\nORDER BY assigned_at;', 'enum_distribution': 'SELECT\n    variant,\n    COUNT(DISTINCT user_id) AS entity_count\nFROM fact_experiment_assignment\nGROUP BY variant\nORDER BY entity_count DESC, variant;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_experiment_assignment base\nLEFT JOIN dim_user ref\n    ON base.user_id = ref.user_id;'}
