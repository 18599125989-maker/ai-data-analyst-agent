...
"""
功能：
验证 knowledge_manual/table_cards_draft/dim_user_role_history.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── dim_user_role_history_validation.json
└── dim_user_role_history_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "dim_user_role_history_validation.json"
MD_OUTPUT = OUTPUT_DIR / "dim_user_role_history_validation.md"

TABLE_NAME = 'dim_user_role_history'
PRIMARY_KEY = ['user_id', 'changed_at']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'user_id', 'ref_table': 'dim_user', 'ref_col': 'user_id'}]
ENUM_COLUMNS = ['old_role', 'new_role']
TIME_COLUMNS = ['changed_at']
NUMERIC_COLUMNS = []
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    changed_at AS time_key,\n    COUNT(*) AS row_count\nFROM dim_user_role_history\nGROUP BY changed_at\nORDER BY changed_at;', 'enum_distribution': 'SELECT\n    old_role,\n    COUNT(DISTINCT user_id) AS entity_count\nFROM dim_user_role_history\nGROUP BY old_role\nORDER BY entity_count DESC, old_role;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM dim_user_role_history base\nLEFT JOIN dim_user ref\n    ON base.user_id = ref.user_id;'}
