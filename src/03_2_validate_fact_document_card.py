...
"""
功能：
验证 knowledge_manual/table_cards_draft/fact_document.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── fact_document_validation.json
└── fact_document_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "fact_document_validation.json"
MD_OUTPUT = OUTPUT_DIR / "fact_document_validation.md"

TABLE_NAME = 'fact_document'
PRIMARY_KEY = ['doc_id']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'tenant_id', 'ref_table': 'dim_tenant', 'ref_col': 'tenant_id'}]
ENUM_COLUMNS = ['doc_type']
TIME_COLUMNS = ['created_at', 'last_edit_at']
NUMERIC_COLUMNS = ['edit_count']
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    created_at AS time_key,\n    COUNT(*) AS row_count\nFROM fact_document\nGROUP BY created_at\nORDER BY created_at;', 'enum_distribution': 'SELECT\n    doc_type,\n    COUNT(DISTINCT doc_id) AS entity_count\nFROM fact_document\nGROUP BY doc_type\nORDER BY entity_count DESC, doc_type;', 'metric_trend': 'SELECT\n    created_at AS time_key,\n    ROUND(SUM(edit_count), 2) AS total_edit_count\nFROM fact_document\nGROUP BY created_at\nORDER BY created_at;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM fact_document base\nLEFT JOIN dim_tenant ref\n    ON base.tenant_id = ref.tenant_id;'}
