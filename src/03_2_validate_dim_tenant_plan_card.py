...
"""
功能：
验证 knowledge_manual/table_cards_draft/dim_tenant_plan.yaml 中的核心假设。

输出：
outputs/knowledge/validation/
├── dim_tenant_plan_validation.json
└── dim_tenant_plan_validation.md
"""
...
JSON_OUTPUT = OUTPUT_DIR / "dim_tenant_plan_validation.json"
MD_OUTPUT = OUTPUT_DIR / "dim_tenant_plan_validation.md"

TABLE_NAME = 'dim_tenant_plan'
PRIMARY_KEY = ['tenant_id', 'effective_from']
NATURAL_KEY = []
FOREIGN_KEYS = [{'column': 'tenant_id', 'ref_table': 'dim_tenant', 'ref_col': 'tenant_id'}, {'column': 'plan_tier', 'ref_table': 'dim_plan', 'ref_col': 'plan_tier'}]
ENUM_COLUMNS = ['plan_name', 'plan_tier']
TIME_COLUMNS = ['effective_from', 'effective_to']
NUMERIC_COLUMNS = []
JSON_COLUMNS = []
SQL_PATTERNS = {'time_distribution': 'SELECT\n    effective_from AS time_key,\n    COUNT(*) AS row_count\nFROM dim_tenant_plan\nGROUP BY effective_from\nORDER BY effective_from;', 'enum_distribution': 'SELECT\n    plan_name,\n    COUNT(DISTINCT tenant_id) AS entity_count\nFROM dim_tenant_plan\nGROUP BY plan_name\nORDER BY entity_count DESC, plan_name;', 'join_coverage_sample': 'SELECT\n    COUNT(*) AS row_count\nFROM dim_tenant_plan base\nLEFT JOIN dim_tenant ref\n    ON base.tenant_id = ref.tenant_id;'}
